                ...
                props.field.map((row, rowIndex) => (
                    <div className={"row"}>
                        {row.map((cell, cellIndex) => (
                            <div className={'cell'}
                                 onClick={() => makeMove(rowIndex, cellIndex)}>{cell === 'X' ? props.player1Icon !== null && String.fromCodePoint(parseInt(props.player1Icon, 16)) : cell === 'O' ? props.player1Icon !== null && String.fromCodePoint(parseInt(props.player2Icon, 16)) : null}<span className={props.winCombo.length === 3 && props.winCombo !== 'tie' &&`${winDirection(rowIndex, cellIndex, props.winCombo)}`}></span></div>
                        ))
                        }
                    </div>
                ))
